...
fetch(API_URL)
  ...
  .then(result => {
    ...
    data.map(element => {
      ...
      const { foto, photo } = element;
      const { tipo , property_type } = element;
      const { nome, name } = element;
      const { preco, price } = element
      ...
      // propertyImage = document.createElement("img");
      // propertyImage.className = "photo";
      // propertyImage.innerHTML = photo;  
